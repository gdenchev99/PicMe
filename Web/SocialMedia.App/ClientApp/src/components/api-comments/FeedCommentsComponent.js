    ...
    return (
        <div className="view-comments">
            ...
            <ul className="feed-comments">
                {params.data.length <= 0 ? <div>No comments yet.</div> : params.data.map(c => <li>
                    <p><span className="comments-username">{c.creatorUserName}</span><span className="comment-text"> {c.text}</span></p>
                </li>)}
            </ul>
